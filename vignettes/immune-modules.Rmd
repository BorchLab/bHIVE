...
package: bHIVE
vignette: >
  %\VignetteIndexEntry{Composing Immune Modules}
...
`AINet$new()` -- mix and match any combination.
...
The **adaptive** method is the most distinctive -- it implements the insight
from Reddy (2026) that somatic hypermutation and the Adam optimizer are the
same algorithm discovered independently by evolution and machine learning.
Each feature dimension maintains running mean and variance of past
gradients.
...
results <- lapply(methods, function(m) {
  ...
  model <- AINet$new(nAntibodies = 15, maxIter = 15, shm = shm,
                     verbose = FALSE)
...
model_idi <- AINet$new(
  ...
  idiotypic = IdiotypicNetwork$new(
    theta_low = 0.01, theta_high = 0.5,
    source_rate = 0.5, timeSteps = 20
  ),
  verbose = FALSE
)
